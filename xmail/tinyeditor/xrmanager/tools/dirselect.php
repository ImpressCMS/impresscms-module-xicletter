<?php
    ...
    echo "<html>";
    echo "<head>";
    echo "<title>"._TINYMGR_TARGETDIR."</title>";
    ...
    echo "</script>";
    echo "</head>";
    echo "<body onload='init();' style='padding: 10px; background-color: #f0f0ee;'>";
    echo "<form action=''>";
    echo "<div class='title'>"._TINYMGR_TARGETDIR."</div>";
    echo "<div class='elements' style='padding-left: 5px;'>";
    echo "<fieldset><label class='pres' style='padding-right: 20px;' for='f_foldername'>"._TINYMGR_DIRECTORY."</label>";
    echo "<select name='dir' class='dirWidth' id='f_foldername'>";
    echo "<option value=\"/\">/</option>";

	foreach ($dirs as $relative=>$fullpath) {
		echo "<option value=\"".rawurldecode($relative)."\">".$relative."</option>";
	}
