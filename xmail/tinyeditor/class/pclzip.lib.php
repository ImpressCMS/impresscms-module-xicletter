...
//
// --------------------------------------------------------------------------------
// $Id: pclzip.lib.php,v 1.1 2006/06/28 10:20:22 frankblacksf Exp $
// --------------------------------------------------------------------------------
	if (file_exists('../xrmanager/xrincludes.php')) include_once '../xrmanager/xrincludes.php';

	if(!defined('XOOPS_ROOT_PATH')) die('XOOPS_ROOT_PATH is not defined!');

	if ($moduleConfig['tinyedmgrftp'] == 1) {
		if (!defined('TINY_FTPSERVER') || !defined('TINY_FTPUSER') || !defined('TINY_FTPPASS') || !defined('TINY_FTPSERVERPLUGIN') || !defined('TINY_FTPUSERPLUGIN') || !defined('TINY_FTPPASSPLUGIN')) {
			include TINY_ROOT_PATH.'include/ftpconfig.php';
			include TINY_ROOT_PATH.'include/ftpconfigplugin.php';
		}
		if (TINY_FTPSERVER == '' || TINY_FTPUSER == '' || TINY_FTPPASS == '' || TINY_FTPSERVERPLUGIN == '' || TINY_FTPUSERPLUGIN == '' || TINY_FTPPASSPLUGIN == '')
...
//      if ($p_entry['compressed_size'] == $p_entry['size'])
  ...
  function privDirCheck($p_dir, $p_is_dir=false)
  {
    ...
    if ($moduleConfig['tinyedmgrftp'] != 1) {
		...
		$p_dir = str_replace(XOOPS_ROOT_PATH, '', $p_dir);
		include_once TINY_ROOT_PATH.'class/ftpclass.php';
		if (preg_match("/tinyeditor/", $p_dir) && preg_match("/plugins/", $p_dir)) {
			$ftp = new ftp();
			$ftp->ftpConnect(TINY_FTPSERVERPLUGIN);
			$ftp->ftpLogin(TINY_FTPUSERPLUGIN, TINY_FTPPASSPLUGIN);
			$ftp->ftpMkdir(TINY_FTPOPTPATHPLUGIN.$p_dir);
			$ftp->ftpSite('CHMOD 0777 '.TINY_FTPOPTPATHPLUGIN.$p_dir.'');
			$ftp->ftpClose();
		} else {
			$ftp = new ftp();
			$ftp->ftpConnect(TINY_FTPSERVER);
			$ftp->ftpLogin(TINY_FTPUSER, TINY_FTPPASS);
			$ftp->ftpMkdir(TINY_FTPOPTPATH.$p_dir);
			$ftp->ftpSite('CHMOD 0777 '.TINY_FTPOPTPATH.$p_dir.'');
			$ftp->ftpClose();			
		}
    }
    // ----- Return
