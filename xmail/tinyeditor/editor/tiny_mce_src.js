...
TinyMCE_Engine.prototype = {
	init : function(settings) {
		...
		if (p.length > 0) {
			...
			if (p[i].charAt(0) != '-' && p[i] != 'owntoolbar') {
this.loadScript(tinyMCE.baseURL + '/plugins/' + p[i] + '/editor_plugin' + tinyMCE.srcMode + '.js');
} else {
this.loadScript(tinyMCE.baseURL + '/plugins/' + p[i] + '/editor_plugin' + tinyMCE.srcMode + '.php');
}
			}
		}
	...
	loadPlugin : function(n, u) {
		...
		Treffer = u.match(/owntoolbar/g);
if(Treffer) {
this.loadScript(u + "/editor_plugin" + (tinyMCE.srcMode ? '_src' : '') + ".php");
} else {
this.loadScript(u + "/editor_plugin" + (tinyMCE.srcMode ? '_src' : '') + ".js");
}
	},
