...
if (file_exists('../../../mainfile.php')) include_once '../../../mainfile.php';
	...
	if (!defined('XOOPS_ROOT_PATH'))
		die('No XOOPS_ROOT_PATH defined in tiny_mce_gzip.php');
	...
	$cachePath = XOOPS_ROOT_PATH.'/cache'; // Cache path, this is where the .gz files will be stored
	...
	function getCurrentUsersGroupsgzip() {
		global $xoopsUser;
		return $xoopsUser ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
	}
		...
		if ($moduleConfig['tinygroupoverride'] != '') {
			...
			foreach ($split_override_array as $override_value) {
				...
				if (array_intersect($getthegroupid, $pieces)) {
					$thegroupid = "_group".$group_pointer;
					break;
				} else {
			...
			$thegroupid = array_slice($getthegroupid, 0, 1);
			$thegroupid = implode(" ", $thegroupid);
			$thegroupid = "_group".trim($thegroupid);
		}
	} else {
		$thegroupid = "_group3";
	}
	...
	*/);
	...
	if ($diskCache) {
		...
		if ($compress)
			$cacheFile = $cachePath . "/tiny_mce_" . $cacheKey . $thegroupid . ".gz";
		else
			$cacheFile = $cachePath . "/tiny_mce_" . $cacheKey . $thegroupid . ".js";
	}
