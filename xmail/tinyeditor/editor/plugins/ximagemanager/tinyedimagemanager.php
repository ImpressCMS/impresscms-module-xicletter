<?php
// $Id: tinyedimagemanager.php,v 1.2 2006/07/10 16:14:17 frankblacksf Exp $
//  ------------------------------------------------------------------------ //
...
if ($op == 'list') {
        ...
        if ($catshow > 0) {
            ...
            if ($total > 0) {
                ...
                if ($total > 10) {
                    ...
                    $nav = new XoopsPageNav($total, 10, $start, 'start', 'cat_id='.$catshow);
    ...
    $xoopsTpl->display('db:tinyed_imagemanager.html');
    exit();
}
...
if ($op == 'upload') {
    ...
    $form = new XoopsThemeForm('', 'image_form', 'tinyedimagemanager.php', 'post', true);
    ...
    $xoopsTpl->assign('lang_close', _CLOSE);
    $xoopsTpl->assign('xoops_url', XOOPS_URL);
    $xoopsTpl->display('db:tinyed_imagemanager2.html');
    exit();
}
...
if ($op == 'doupload') {
    ...
    header('location: tinyedimagemanager.php?cat_id='.$imgcat_id.'&target='.$target);
}
