   ...
   if( troca_var($filename,$filename_tmp,$origem, $destino)) {
      ...
      if( troca_var($filename_antes,$filename_antes_tmp,$origem, $destino )) {
           ...
           //-------------------------------------------------
            ...
            //---------------------------------------------------------------------------
            //Se a definição do esquema da sua base de dados precisa de valores de
            //variáveis definidos no momento de instalação, defina esses valores aqui.
            //No esquema de exemplo acima não foram usadas quaisquer variáveis.
            //Portanto, a lista de variáveis foi definida como um array vazio.
            //---------------------------------------------------------------------------
            ...
            if(!$sucesso) {
                echo "<div class='errorMsg'> Error: ".$gestor->error. "</div>";
               	...
               	if (!copy($filename,$filename_antes)) {
                    $gestor->error[]="Falha na copia de $filename para $filename_antes , verifique as permissões da pasta ";
               	}

            }

            //---------------------------------------------------------------------------
            //Se o procedimento de instalação falhou,
...
<?php
 ...
 if($id_arq=@fopen($filename,"r")) {
    ...
    fclose($id_arq);
       ...
       echo "<script>alert('Não foi possível abrir script de criação do banco $filename , verifique as permissões da pasta');</script>";
       return false;
 }
  ...
  if($id_arq=@fopen($filename_tmp,"w")) {
     ...
     echo "<script>alert('Falha na geração do $filename_tmp , verifique as permissões da pasta ');</script>";
     return false;
  }
  return true;
}
